...
#ifdef WIN32
  timer->id = SetTimer( NULL, 0, interval, NULL );
#endif
...
#elif defined( WIN32 )
static void
handleTimeouts( unsigned int id )
{
  GLUTtimer *timer, *prev;
...
LONG WINAPI
__glutProcessEvents(HWND hWnd, UINT message, DWORD wParam, LONG lParam)
{
    PAINTSTRUCT paint;
    HDC hdc;
	RECT winpos;
	...
	switch (message) {
		...
		case WM_MOVE:
			if(window->ctx && window->renderCtx)
        ...
        if (wParam & MK_LBUTTON || wParam & MK_MBUTTON || wParam & MK_RBUTTON) 
		{
			...
			{
				__glutSetWindow( window );
            	(*window->motion)(LOWORD(lParam), HIWORD(lParam));
			}
		}
		else if (window->passive)
		{
			__glutSetWindow( window );
            (*window->passive)(LOWORD(lParam), HIWORD(lParam));
		}
        return (0);
      ...
      case WM_RBUTTONDOWN:
		...
		switch( message )
		{
		  case WM_LBUTTONDOWN: button = GLUT_LEFT_BUTTON; break;
		  case WM_MBUTTONDOWN: button = GLUT_MIDDLE_BUTTON; break;
		  case WM_RBUTTONDOWN: button = GLUT_RIGHT_BUTTON; break;
		}
      ...
      case WM_RBUTTONUP:
		...
		switch( message )
		{
		  case WM_LBUTTONUP: button = GLUT_LEFT_BUTTON; break;
		  case WM_MBUTTONUP: button = GLUT_MIDDLE_BUTTON; break;
		  case WM_RBUTTONUP: button = GLUT_RIGHT_BUTTON; break;
		}
		if (window->mouse)
        {
			__glutSetWindow( window );
            (*window->mouse)(button, GLUT_UP,
            	LOWORD(lParam), HIWORD(lParam));
		}
		// reset the modifier key mask
		__glutModifierMask = (unsigned int)~0;
	...
	case WM_SYSKEYDOWN:
        ...
        if (key && window->special)
        {
			__glutSetWindow( window );
		  GetWindowRect(window->win, &winpos);
		  GetCursorPos(&mousepos);
		  x = mousepos.x - winpos.left;
		  y = mousepos.y - winpos.top;
      ...
      case WM_CHAR:
        ...
        if (key && window->keyboard)
        {
		  ...
		  (*window->keyboard)((unsigned char)key, x, y);
        }
		__glutModifierMask = (unsigned int)~0;
        return (0);
	  ...
	  case WM_SHOWWINDOW:
		__glutSetWindow ( window );
		window->visState = wParam;

		if (window->visState)
		{
			window->shownState = SW_SHOWNORMAL;
			if (window->visibility)
            	(*window->visibility)(GLUT_VISIBLE);
		}
		else
		{
			window->shownState = SW_HIDE;
			if (window->visibility)
            	(*window->visibility)(GLUT_NOT_VISIBLE);
		}
		glutPostRedisplay();
		return (0);

	  case WM_TIMER:
		  handleTimeouts( wParam );
		  return 0;
    }
  ...
  for (;;) {
	 ...
	 */
		...
		else 
		{
			...
			/*
			 *  Process all messages in a blocking manner
			 */

			while ( !(__glutIdleFunc || __glutWindowWorkList) )
			{
				if (GetMessage(&Message, NULL, 0, 0) )
				{
					if ( Message.message == WM_TIMER )
