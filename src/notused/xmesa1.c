/* $Id: xmesa1.c,v 1.37 1997/08/26 02:08:42 brianp Exp $ */
...
XMesaContext XMesaCreateContext( XMesaVisual v, XMesaContext share_list )
{
   ...
   if (fxEnvVar) {
      if (v->undithered_pf==PF_5R6G5B && tolower(fxEnvVar[0])=='w') {
         /* Window rendering */
         int attribs[] = {
            FXMESA_DEPTH_SIZE, 1,
            FXMESA_NONE
         ...
         c->FXhack = c->FXctx ? GL_TRUE : GL_FALSE;
      }
      else if (tolower(fxEnvVar[0])=='f') {
         ...
         c->FXctx = fxMesaCreateBestContext(0, FX_WIDTH, FX_HEIGHT, attribs);
         c->FXuse = c->FXctx ? GL_TRUE : GL_FALSE;
         c->FXhack = GL_FALSE;
      }
   }
#endif
...
static void FXgetImage( XMesaBuffer b )
{
   ...
   if (b->width != width || b->height != height) {
      ...
      if (b->width & 1)  b->width--;  /* prevent odd width */
      xmesa_alloc_back_buffer( b );
   }
