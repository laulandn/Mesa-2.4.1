...
{
	long		ret = (hWNDOldProc)(hwnd,message,wParam,lParam);
	...
	if(ctx && hwnd == hWND)
	{
		switch(message)
		{
			...
			case WM_SIZE:
				...
				{
					MessageBox(0,_T("Error changing window size"),_T("fxMESA"),MB_OK);
					PostMessage(hWND,WM_CLOSE,0,0);
				}
				break;
...
DllExport BOOL APIENTRY wglCopyContext(HGLRC hglrcSrc,HGLRC hglrcDst,UINT mask)
{
	return(FALSE);
}

DllExport HGLRC APIENTRY wglCreateContext(HDC hdc)
{
	HWND		hWnd;
	WNDPROC	oldProc;
	int			error;
...
DllExport HGLRC APIENTRY wglCreateLayerContext(HDC hdc,int iLayerPlane)
{
	SetLastError(0);
	return(NULL);
}

DllExport BOOL APIENTRY wglDeleteContext(HGLRC hglrc)
{
...
DllExport HGLRC APIENTRY wglGetCurrentContext(VOID)
{
	if(ctx)
		return((HGLRC)1);
	SetLastError(0);
...
DllExport HDC APIENTRY wglGetCurrentDC(VOID)
{
	if(ctx)
		return(hDC);
	SetLastError(0);
...
DllExport PROC APIENTRY wglGetProcAddress(LPCSTR lpszProc)
{
	int		i;
...
DllExport BOOL APIENTRY wglMakeCurrent(HDC hdc,HGLRC hglrc)
{
...
DllExport BOOL APIENTRY wglShareLists(HGLRC hglrc1,HGLRC hglrc2)
{
...
DllExport BOOL APIENTRY wglUseFontBitmapsA(HDC hdc,DWORD first,DWORD count,DWORD listBase)
{
	return(FALSE);
}

DllExport BOOL APIENTRY wglUseFontBitmapsW(HDC hdc,DWORD first,DWORD count,DWORD listBase)
{
	return(FALSE);
}

DllExport BOOL APIENTRY wglUseFontOutlinesA(HDC hdc,DWORD first,DWORD count,
...
DllExport BOOL APIENTRY wglUseFontOutlinesW(HDC hdc,DWORD first,DWORD count,
										  DWORD listBase,FLOAT deviation,
										  FLOAT extrusion,int format,
										  LPGLYPHMETRICSFLOAT lpgmf)
{
...
#ifdef NEWMSC
DllExport BOOL APIENTRY wglDescribeLayerPlane(HDC hdc,int iPixelFormat,
...
DllExport int APIENTRY wglSetLayerPaletteEntries(HDC hdc,int iLayerPlane,
...
DllExport int APIENTRY wglGetLayerPaletteEntries(HDC hdc,int iLayerPlane,
											   int iStart,int cEntries,
											   COLORREF *pcr)
{
	SetLastError(0);
...
DllExport BOOL APIENTRY wglRealizeLayerPalette(HDC hdc,int iLayerPlane,BOOL bRealize)
{
	SetLastError(0);
	return(FALSE);
}
...
DllExport BOOL APIENTRY wglSwapLayerBuffers(HDC hdc,UINT fuPlanes)
{
...
DllExport int APIENTRY wglDescribePixelFormat(HDC hdc,int iPixelFormat,UINT nBytes,
...
DllExport BOOL APIENTRY wglSwapBuffers(HDC hdc)
{
	if(!ctx)
	{
		SetLastError(0);
